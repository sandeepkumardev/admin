...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body || !body.courierOrderId || !body.orderNumber || !body.platform) {
    ...
    const result = await new CourierClass().cancelOrder(body.courierOrderId, body.orderNumber);

    if (!result || !result.data) {
      console.log([`CANCEL ORDER - ${platform}`], result);
      return NextResponse.json({ ok: false, error: "Failed to cancel order" });
    }
